...
describe('production deployment credentials', () => {
  ...
  it('publishes links for the current release', () => {
    ...
    expect(page).toContain('/downloads/PlainList-2.3.1.apk');
    expect(page).toContain('/downloads/PlainList-2.3.1-arm64.dmg');
    expect(page).toContain('/downloads/PlainList-2.3.1-x64.dmg');
    expect(page).not.toContain('PlainList-2.3.0');
  });
});
